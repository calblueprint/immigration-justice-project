...
const locationMatch : MatchField<CaseListing> = {
  getMatch: (data, profileData) => data.client_location ? data.client_location === profileData.location : undefined,
  getText: (data, profileData, match) => {
    console.log(match)
    return match === undefined ? 'No information available about Client Location' : `This client is ${match ? '' : 'not '}in your city.`
  }
};
    ...
    <Flex $direction="column" $gap="30px">
      ...
      <Flex $direction="column" $gap="15px">
        ...
        <Flex $gap='16px'>
          ...
          <P>You {matchedLanguages.length > 0 ? '' : "don't "}list {matchedLanguages.length > 0 ? parseLanguageList(matchedLanguages, false) : parseLanguageList(listingData.languages, true)} in your languages.</P>
        </Flex>
      </Flex>
      <Flex>
